...
const makeRepo = () => new SqliteReservationRepository(new DatabaseSync(":memory:"));

describe("SqliteReservationRepository", () => {
  it("save 후 findById로 (상태 포함) 재구성한다", async () => {
    const repo = makeRepo();
    const reservation = new Reservation("showtime-1", new Seat("A", 7), "hoon@example.com", "res-1");
  ...
  it("활성(CONFIRMED) 예약이 있는 좌석을 찾는다", async () => {
    ...
    await repo.save(new Reservation("showtime-1", seat, "hoon@example.com", "res-1"));
  ...
  it("취소된 예약만 있는 좌석은 활성 좌석이 아니다", async () => {
    const repo = makeRepo();
    const seat = new Seat("A", 7);
    const reservation = new Reservation("showtime-1", seat, "hoon@example.com", "res-1");
    reservation.cancel();
    await repo.save(reservation);
