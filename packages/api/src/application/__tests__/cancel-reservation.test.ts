...
describe("CancelReservationUseCase", () => {
  const makeScenario = async () => {
    ...
    const reservation = new Reservation("showtime-1", new Seat("A", 7), "hoon@example.com");
  ...
  it("존재하지 않는 예약 취소는 ReservationNotFoundError를 던진다", async () => {
    ...
    await expect(useCase.execute("존재하지않는-아이디")).rejects.toBeInstanceOf(ReservationNotFoundError);
  });
